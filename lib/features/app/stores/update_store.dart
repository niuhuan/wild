...
class UpdateStore {
  ...
  UpdateStore() : _state = signals.signal(const UpdateState());

  final signals.Signal<UpdateState> _state;
  UpdateState get state => _state.value;
  signals.Signal<UpdateState> get signal => _state;
  ...
  Future<VersionInfo?> checkUpdate({bool force = false}) async {
    ...
    try {
      ...
      // 即使没有更新，也标记为已检查
      _state.value = state.copyWith(hasCheckedOnStartup: true);
    } catch (e) {
      if (kDebugMode) {
