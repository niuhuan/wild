...
class BookshelfStore {
  ...
  final signals.Signal<BookshelfState> _state;
  BookshelfState get state => _state.value;
  signals.Signal<BookshelfState> get signal => _state;

  Future<void> loadBookcases() async {
    try {
      _state.value = state.copyWith(status: BookshelfStatus.loading);
  ...
  void selectBookcase(String caseId) {
    _state.value = state.copyWith(currentCaseId: caseId);
  }
  ...
  void toggleBookSelection(String bid) {
    ...
    _state.value = state.copyWith(selectedBids: newSelectedBids);
  }
  ...
  Future<void> addToBookshelf(String aid) async {
    try {
      ...
      if (state.bookcases.isNotEmpty) {
        ...
        _state.value = state.copyWith(bookcaseContents: newContents, tip: tip);
      }
    } catch (e) {
      _state.value = state.copyWith(
        status: BookshelfStatus.error,
  ...
  Future<void> removeFromBookshelf(String aid) async {
    try {
      ...
      _state.value = state.copyWith(bookcaseContents: contents, tip: tip);
    } catch (e) {
      _state.value = state.copyWith(
        status: BookshelfStatus.error,
        errorMessage: e.toString(),
