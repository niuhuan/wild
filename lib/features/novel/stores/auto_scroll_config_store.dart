...
class AutoScrollConfigStore {
  ...
  final signals.Signal<AutoScrollConfig> _state;
  AutoScrollConfig get state => _state.value;
  signals.Signal<AutoScrollConfig> get signal => _state;
  ...
  Future<void> updateScrollSpeed(double speed) async {
    try {
      ...
      _state.value = state.copyWith(scrollSpeed: speed);
    } catch (e) {
      // 如果保存失败，仍然更新状态
      _state.value = state.copyWith(scrollSpeed: speed);
    }
  ...
  Future<void> updateScrollInterval(int interval) async {
    try {
      ...
      _state.value = state.copyWith(scrollInterval: interval);
    } catch (e) {
      // 如果保存失败，仍然更新状态
      _state.value = state.copyWith(scrollInterval: interval);
    }
