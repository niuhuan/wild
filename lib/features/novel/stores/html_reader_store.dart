...
class HtmlReaderStore {
  ...
  }) : _state = signals.signal<HtmlReaderState>(const HtmlReaderLoading());

  final signals.Signal<HtmlReaderState> _state;
  HtmlReaderState get state => _state.value;
  signals.Signal<HtmlReaderState> get signal => _state;
  ...
  Future<void> loadChapter({String? aid, String? cid}) async {
    try {
      _state.value = const HtmlReaderLoading();
      ...
      if (chapterTitle == null) {
        _state.value = const HtmlReaderError('章节不存在');
        return;
      }
      ...
      _state.value = HtmlReaderError(e.toString());
    }
  }
